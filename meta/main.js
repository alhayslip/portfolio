...
function updateFileDisplay(commitArray) {
  const lines = commitArray.flatMap(d => d.lines);
...
function setupScatterScrolly() {
  ...
  .onStepEnter(response => {
    ...
    updateScatterPlot([commit]); // only 1 point at a time
  });
}


//Return scrolly two file unit visualization//
function setupFileScrolly() {


  const commitCopy = commits.map(d => structuredClone(d));
  ...
  .onStepEnter(response => {
    const commit = response.element.__data__;
    updateFileDisplay([commit]); // Only show that commit's file viz
  });
}
